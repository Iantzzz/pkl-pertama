<x-app-layout>
    <x-slot name="header">
        <h1 class="text-lg font-semibold text-warm-900 dark:text-warm-50">Edit Laporan PKL</h1>
    </x-slot>
        ...
        <div class="bg-white dark:bg-warm-800 rounded-xl border border-warm-200 dark:border-warm-700 shadow-sm p-6 sm:p-8">
            <form action="{{ route('laporan.update', $laporan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="5" class="mt-1.5 block w-full rounded-lg border border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800 text-warm-900 dark:text-warm-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" required>{{ old('deskripsi', $laporan->deskripsi) }}</textarea>
                ...
                <div>
                    ...
                    @if($laporan->foto)
                        <div class="mt-2 mb-3 p-3 bg-warm-50 dark:bg-warm-700/30 rounded-xl border border-warm-200 dark:border-warm-700">
                            <img src="/{{ $laporan->foto }}" class="w-24 h-24 object-cover rounded-lg border border-warm-200 dark:border-warm-700 cursor-pointer hover:opacity-80 transition-opacity" onclick="openLightbox('/{{ $laporan->foto }}')">
                            <p class="text-xs text-warm-500 dark:text-warm-400 mt-1.5">Foto saat ini. Upload foto baru untuk mengganti.</p>
                        </div>
                    @endif
                    <div class="mt-1.5 flex items-center justify-center w-full">
                        <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-warm-200 dark:border-warm-700 rounded-xl cursor-pointer bg-warm-50 dark:bg-warm-700/30 hover:bg-blue-50 dark:hover:bg-blue-500/5 hover:border-blue-500/50 transition-all">
                            <div class="flex flex-col items-center justify-center pt-4 pb-5">
                                ...
                                <p class="text-xs text-warm-500 dark:text-warm-400">Klik untuk ganti foto (maks 2MB, JPG/PNG)</p>
                            </div>
