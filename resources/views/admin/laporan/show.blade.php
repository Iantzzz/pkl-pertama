<x-app-layout>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-6">
        ...
        <a href="{{ route('admin.laporan.index') }}" class="inline-flex items-center text-sm text-warm-500 dark:text-warm-400 hover:text-warm-700 dark:hover:text-warm-200 transition-colors">
        ...
        <div class="bg-white dark:bg-warm-800 rounded-xl border border-warm-200 dark:border-warm-700 shadow-sm p-6 sm:p-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="block text-xs font-semibold text-warm-500 dark:text-warm-400 uppercase tracking-wider mb-1">Siswa</label>
                    <p class="text-warm-900 dark:text-warm-50 font-medium">{{ $laporan->user->name }}</p>
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="text-warm-700 dark:text-warm-300 whitespace-pre-line bg-warm-50 dark:bg-warm-700/20 rounded-xl p-4 border border-warm-200 dark:border-warm-700">{{ $laporan->deskripsi }}</div>
                </div>
                <div>
                    ...
                    @endphp
                    <span class="px-2.5 py-1 rounded-full text-xs font-semibold tracking-wide inline-block {{ $statusColors[$laporan->status] ?? 'bg-warm-100 text-warm-600 dark:bg-warm-700 dark:text-warm-300' }}">
                ...
                <div class="sm:col-span-2">
                    ...
                    <p class="text-warm-600 dark:text-warm-400 bg-warm-50 dark:bg-warm-700/20 rounded-xl p-4 border border-warm-200 dark:border-warm-700">{{ $laporan->catatan_guru }}</p>
                </div>
                @endif
                <div class="sm:col-span-2">
            ...
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="p-4 bg-warm-50 dark:bg-warm-700/20 rounded-xl border border-warm-200 dark:border-warm-700">
                    ...
                    <form action="{{ route('laporan.status', $laporan) }}" method="POST" class="flex gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-gradient text-white font-semibold text-xs rounded-lg shadow-sm hover:shadow transition-all">Simpan</button>
                    </form>
                </div>
                ...
                <div class="p-4 bg-warm-50 dark:bg-warm-700/20 rounded-xl border border-warm-200 dark:border-warm-700">
                    ...
                    <form action="{{ route('laporan.nilai', $laporan) }}" method="POST" class="space-y-3">
                        ...
                        <div class="flex gap-3 items-start">
                            <div>
                                <label class="block text-xs text-warm-500 dark:text-warm-400 mb-1">Nilai (0-100)</label>
                                <input type="number" name="nilai" min="0" max="100" value="{{ old('nilai', $laporan->nilai) }}" class="w-20 rounded-lg border border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800 text-sm text-warm-900 dark:text-warm-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all">
                            </div>
                            <div class="flex-1">
                                <label class="block text-xs text-warm-500 dark:text-warm-400 mb-1">Catatan</label>
                                <textarea name="catatan_guru" rows="2" class="block w-full rounded-lg border border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800 text-sm text-warm-900 dark:text-warm-50 placeholder-warm-400 dark:placeholder-warm-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all" placeholder="Catatan untuk siswa...">{{ old('catatan_guru', $laporan->catatan_guru) }}</textarea>
                            </div>
                        </div>
                        <button type="submit" class="px-4 py-2 bg-blue-gradient text-white font-semibold text-xs rounded-lg shadow-sm hover:shadow transition-all">Simpan Nilai</button>
                    </form>
                </div>
            </div>

            <div class="mt-6 flex gap-2">
                <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'hapus-laporan')" class="inline-flex items-center px-4 py-2 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 font-medium text-xs rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-all cursor-pointer">
                ...
                <x-modal name="hapus-laporan" :show="false" focusable>
                    <div class="p-6">
                        ...
                        <p class="mt-2 text-sm text-warm-500 dark:text-warm-400">Apakah Anda yakin ingin menghapus laporan "<strong>{{ $laporan->kegiatan }}</strong>" milik <strong>{{ $laporan->user->name }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
        ...
        <div class="bg-white dark:bg-warm-800 rounded-xl border border-warm-200 dark:border-warm-700 shadow-sm p-6 sm:p-8">
            <h3 class="text-base font-semibold text-warm-900 dark:text-warm-50 mb-5">Komentar</h3>
                        ...
                        <div class="p-4 bg-warm-50 dark:bg-warm-700/20 rounded-xl border border-warm-200 dark:border-warm-700">
                            <div class="flex justify-between items-start mb-1">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-full bg-blue-gradient flex items-center justify-center">
                                        <span class="text-xs font-bold text-white">{{ substr($comment->user->name, 0, 1) }}</span>
                                    </div>
                                    <span class="font-medium text-sm text-warm-900 dark:text-warm-50">{{ $comment->user->name }}</span>
                                </div>
                                <span class="text-xs text-warm-400 dark:text-warm-500">{{ $comment->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="mt-2 text-sm text-warm-600 dark:text-warm-300 ml-9">{{ $comment->isi }}</p>
                        </div>
                    @endforeach
                </div>
            @else
